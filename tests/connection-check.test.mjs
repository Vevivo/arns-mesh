...
test('connection checks use bounded validation-only requests and identify protocol failures',async()=>{
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  if(req.url==='/mesh/v1/query')res.end(JSON.stringify({ok:false,error:'invalid_arns_name'}));
 ...
 try{
  ...
  assert.deepEqual(seen.find(r=>r.path==='/mesh/v1/query').body,{op:'snapshot',name:''});
