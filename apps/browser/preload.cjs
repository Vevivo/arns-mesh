...
contextBridge.exposeInMainWorld('arnsMesh',{
 ...
 inspectNetwork:code=>ipcRenderer.invoke('inspect-network',code),joinNetwork:ticket=>ipcRenderer.invoke('join-network',ticket),refreshNetwork:()=>ipcRenderer.invoke('refresh-network'),stopNetworkUpdates:()=>ipcRenderer.invoke('stop-network-updates'),
 importProfile:mode=>ipcRenderer.invoke('import-profile',mode),exportProfile:()=>ipcRenderer.invoke('export-profile'),checkConnections:()=>ipcRenderer.invoke('check-connections'),getSettings:()=>ipcRenderer.invoke('get-settings'),saveSettings:data=>ipcRenderer.invoke('save-settings',data),panel:open=>ipcRenderer.invoke('panel',open),
