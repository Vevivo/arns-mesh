...
profile="${1:-}"
network_code=""
if [[ "$profile" == '--network' && "$#" == 2 ]]; then
  network_code="$2"
elif [[ "$#" != 1 || -z "$profile" || ! -f "$profile" ]]; then
  echo 'Usage: bash scripts/install-peer.sh PROFILE.json OR bash scripts/install-peer.sh --network CONNECTION_CODE' >&2
  exit 1
fi
...
if [[ -n "$network_code" ]]; then
  "$node_bin" "$source_root/scripts/network.mjs" check-code "$network_code"
else
...
if [[ ! -f "$install_root/data/solana-rpc-seeds.json" && ! -f "$install_root/data/mesh-ip-peers.json" ]]; then
  if [[ -n "$network_code" ]]; then
    "$node_bin" "$stage_root/app/scripts/network.mjs" join "$network_code" --data "$install_root/data"
  else
