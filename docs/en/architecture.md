...
| Target → location | Local/peer hints, sparse historical published indexes, bounded raw-ledger discovery on supporters | Complete or current location coverage |
...
The Node access path rejects DNS/domain requests, HTTPS/SNI, arbitrary URL fetches and gateway routes. The desktop separately restricts page requests and background networking. These are implemented controls and automated-test subjects; **a complete OS-level egress acceptance test across Chromium, WebRTC, IPv6 and existing sockets is still pending**. Application logs alone do not certify that no other process traffic occurred.
...
- **Saved local content:** explicit saved mode may use a dated retained name observation and saved bytes. It does not assert latest chain state.
