...
export class SitePinner{
 ...
 start(name){
  ...
  const snapshot=this.snapshots.get(name);if(!snapshot)throw new Error('saved_name_unavailable');
 ...
 async run(name,snapshot){
  const row=this.rows[name]={name,rootDataId:snapshot.txId,observedAt:snapshot.observedAt,status:'saving',scope:'document',total:1,saved:0,failed:0,errors:[]};this.save();
